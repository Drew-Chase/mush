...
fn parse(args: &[&str]) -> CpConfig {
    ...
    CpConfig::from_args(&owned).expect("should not be --help/--version")
}
...
#[test]
fn combined_rf() {
    let config = parse(&["-rf", "dir", "dest"]);
    assert!(config.recursive);
    assert_eq!(config.overwrite, OverwriteMode::Force);
}

#[test]
fn combined_riv_last_wins() {
    let config = parse(&["-riv", "a", "b"]);
    assert!(config.recursive);
    assert_eq!(config.overwrite, OverwriteMode::Interactive);
    assert!(config.verbose);
}
...
#[test]
fn combined_fin_last_wins() {
    let config = parse(&["-fin", "a", "b"]);
    assert_eq!(config.overwrite, OverwriteMode::NoClobber);
}
...
fn help_returns_none() {
    ...
    assert!(CpConfig::from_args(&owned).is_none());
}
