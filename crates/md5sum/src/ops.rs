...
pub fn format_hash(hash: &str, filename: &str, config: &Md5sumConfig) -> String {
    if config.tag {
...
pub fn check_file(path: &Path, config: &Md5sumConfig) -> io::Result<(usize, usize)> {
    ...
    for line in reader.lines() {
        ...
        let (expected_hash, filename) = if let Some(rest) = line.strip_prefix("MD5 (") {
            ...
            if let Some(paren_pos) = rest.find(") = ") {
                ...
                if config.warn {
                    eprintln!("md5sum: {}: improperly formatted checksum line", path.display());
                }
                continue;
            }
        } else if let Some((hash, fname)) = line.split_once("  ").or_else(|| line.split_once(" *"))
        {
            ...
            if config.warn {
                eprintln!(
                    "md5sum: {}: improperly formatted checksum line",
                    path.display()
                );
            }
            continue;
